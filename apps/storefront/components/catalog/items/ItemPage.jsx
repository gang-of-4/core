...
export default function ItemPage({ item }) {
  ...
  function updateActiveVariant(newOptions) {
    const newActiveVariant = item.variants.find((variant) => {
      if (variant.quantity > 0) {
        return variant.options.every(
          (option) => newOptions[option.group_id] === option.id
        );
      }
    });
    ...
    if (newActiveVariant) {
      ...
      setError(
        `No ${config.catalog.variant.name} found for selected ${config.catalog.option.plural}. Please select a different combination.`
      );
              ...
              <Grid item xs={12} md={7}>
                <Stack spacing={4}>
                  ...
                  <Typography variant="h6">
                    {formatPrice({
                      price: activeVariant?.price || item.price,
                      currency: activeVariant?.currency || item.currency,
                    })}
                  </Typography>
