...
class CatalogApi {
  ...
  async createOptionGroup({ title, type, options }) {
    let data;
    try {
      const { data: returnedData } = await fetchApi({
        url: `/admin/api/catalog/options/group`,
  ...
  async editOptionGroup({ id, title, type, options }) {
    let data;
    try {
      const { data: returnedData } = await fetchApi({
        url: `/admin/api/catalog/options/group/${id}`,
  ...
  async addOption({ groupId, label, value }) {
    let data;
    try {
      const { data: returnedData } = await fetchApi({
  ...
  async editCategory({ id, name, description, banner, logo }) {
    let data;
    try {
      const { data: returnedData } = await fetchApi({
        url: `/admin/api/catalog/categories/${id}`,
