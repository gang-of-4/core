...
const Page = () => {
  ...
  return (
    <>
      <Head>
        <title>
          Admin Dashboard | Edit {capitalize(config.catalog.optionGroup.name)}
        </title>
      </Head>
      <Box
        component="main"
      ...
      >
        <Container maxWidth="xl">
          <Card elevation={16}>
            ...
            <CardHeader
              sx={{ pb: 0 }}
              title={`Edit ${capitalize(config.catalog.optionGroup.plural)}: ${
                optionGroup?.title
              }`}
            />
            <CardContent>
              {optionGroup && <OptionEditForm group={optionGroup} />}
            </CardContent>
          </Card>
        </Container>
      </Box>
